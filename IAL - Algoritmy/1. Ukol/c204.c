...
void untilLeftPar( Stack *stack, char *postfixExpression, unsigned *postfixExpressionLength ) {
    char top;
    //while prochází stack dokud nenarazí na "("
    while (!Stack_IsEmpty(stack)){
            ...
            //předávám hodnoty na výstup
        postfixExpression[*postfixExpressionLength] = top;
        (*postfixExpressionLength)++;
        //použitou hodnotu odstraním
        Stack_Pop(stack);
    }
...
void doOperation( Stack *stack, char c, char *postfixExpression, unsigned *postfixExpressionLength ) {
    ...
    //přiřadím prioritu operátoru na vrcholku zásobníku kdy není prázdný
    ...
    //přiřadím prioritu operátoru se kterým pracuji
    if (c == '/' || c == '*') {
        priorityC = 2;
    }else if (c == '+' || c == '-') {
        priorityC = 1;
    ...
    //podmínka pro prázdný zásobník
    if (Stack_IsEmpty(stack)) {
        Stack_Push(stack,c);
        return;
    } else {

        //porovnám priority "*" a "/" mají větší prio.
...
char *infix2postfix( const char *infixExpression ) {
    ...
    int i = 0;
    unsigned j = 0;
    //čtu dokud není infix prázdný
    while (infixExpression[i] != '\0') {

        //Hledání začátku závorky
        if(infixExpression[i] == '(') {
            Stack_Push(stack,infixExpression[i]);
        }

        //kontrola jestli je znak číslo
        ...
        //kontrola jestli je znak operátor
        ...
        //kontrola ukončení závorky a vyprázdnění stacku po závorku
        if(infixExpression[i] == ')') {
            untilLeftPar(stack, postfixExpression, &j);
        }

        //kontrola konce infixu
        if(infixExpression[i] == '=') {
            char top;
            while (!Stack_IsEmpty(stack)) {
                Stack_Top(stack,&top);
