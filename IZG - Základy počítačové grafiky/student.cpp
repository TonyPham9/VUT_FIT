...
void pinedaPolygon(const Point* points, const int size, const RGBA& color1, const RGBA& color2)
{
    ...
    for (int i = 0; i < size; i++) {
        if (i == size - 1) {
            edge_params[i].deltaX = points[0].x - points[i].x;
            edge_params[i].deltaY = points[0].y - points[i].y;
        }
        else {
            edge_params[i].deltaX = points[i + 1].x - points[i].x;
            edge_params[i].deltaY = points[i + 1].y - points[i].y;
        }
        edge_fnc_values[i] = (minY - points[i].y) * edge_params[i].deltaX - (minX - points[i].x) * edge_params[i].deltaY;
    }
    ...
    for (int y = minY; y < maxY; y++) {
        ...
        for (int x = startX; x != endX; x += stepX) {
            ...
            if (x != endX - stepX) {
                for (int i = 0; i < size; i++) {
                    edge_fnc_values[i] += even ? -(edge_params[i].deltaY) : edge_params[i].deltaY;
                }
            }
        }
        for (int i = 0; i < size; i++) {
            edge_fnc_values[i] += edge_params[i].deltaX;
        }
    }
